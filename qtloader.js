...
function QtLoader(config)
{
    ...
    if (config.containerElements !== undefined) {
        ...
        config.progressHandler = config.progressHandler || function(loadedBytes, totalBytes) {}
    }
    ...
    function fetchCompileWasm(filePath) {
        return fetchResource(filePath).then(function(response) {
            if (typeof WebAssembly.compileStreaming !== "undefined") {
                ...
                var contentLength = response.headers.get('x-amz-meta-decompressedcontentlength');
                ...
                var res = new Response(new ReadableStream({
                    async start(controller) {
                        ...
                        for (;;) {
                            ...
                            if (done)
                            {
                                config.progressHandler(total, total)
                                break
                            }

                            loaded += value.byteLength;
                            config.progressHandler(loaded, total)
